...
- Jeśli ramka **Pewność: NISKA** jest oznaczona, uwzględnij to w sekcji "Otwarte Ryzyka i Założenia" planu i zadaj JEDNO pytanie wyjaśniające, jak postępować (najpierw zweryfikuj, lub zaplanuj z uznanym ryzykiem).
...
2. **Uruchom początkowe zadania badawcze w celu zebrania kontekstu** (pomiń lub zawęź na podstawie Kroku 1.0):
   ...
   Ci agenci będą:
   - Znajdź odpowiednie pliki źródłowe, konfiguracje i testy
   - Śledź przepływ danych i kluczowe funkcje
   - Zwróć szczegółowe wyjaśnienia z odniesieniami file:line
...
6. **Zadawaj głębokie, dociekliwe pytania za pomocą AskUserQuestion**:
   ...
   **O czym NIE pytać:**
   ...
   - Niskopoziomowe szczegóły implementacji, które możesz ustalić samodzielnie (na podstawie badań bazy kodu dla oprogramowania, na podstawie plików kontekstowych i wcześniejszych prac dla nie-oprogramowania)
...
1. **Badaj wzorce implementacji i wcześniejsze prace**:
   ...
   **Dla zadań nie-oprogramowania**, badaj pliki kontekstowe i wcześniejsze prace:
   ...
   - Co zadziałało dobrze (lub nie) w poprzednich iteracjach?
...
4. **Uruchom równoległe podzadania do kompleksowych badań**:
   ...
   **Dla kontekstu historycznego:**
   ...
   Każdy agent będzie:
   - Znajdź odpowiednie pliki i wzorce kodu
   - Zidentyfikuj konwencje i wzorce do naśladowania
   - Szukaj punktów integracji i zależności
   - Zwróć konkretne odniesienia file:line
   - Znajdź testy i przykłady
...
2. **Użyj tej struktury szablonu** (bloki faz zawierają zwykłe wypunktowania — `- ` zamiast `- [ ]` — a pojedyncza kanoniczna sekcja `## Postęp` na dole zawiera stan pól wyboru, zobacz `references/progress-format.md` dla umowy):
...
To NIE jest miejsce do wstępnego decydowania o implementacji. Domyślnie: **pomijaj** całą sekcję. Uwzględnij nagłówek poniżej TYLKO wtedy, gdy coś naprawdę zaskakującego lub nośnego ma zastosowanie — i napisz 1-3 zdania, a nie szablony wypunktowań.

- **Czas i cykl życia** — uwzględnij tylko wtedy, gdy istnieje nieoczywista kolejność, wyścig lub hak cyklu życia, który implementator mógłby inaczej przeoczyć.
- **Specyfikacja doświadczenia użytkownika** — uwzględnij tylko wtedy, gdy zachowanie widoczne dla użytkownika ma ograniczenia, których nie można wywnioskować z wymagań użytkownika (np. specyficzne zarządzanie fokusem, zachowanie przewijania).
- **Ograniczenia wydajności** — uwzględnij tylko wtedy, gdy istnieje rzeczywisty budżet wydajności lub znany punkt krytyczny; pomiń ogólne porady typu "użyj memoizacji".
- **Sekwencjonowanie stanu** — uwzględnij tylko wtedy, gdy kolejność zmian stanu ma znaczenie, a oczywista kolejność jest błędna.
- **Debugowanie i obserwowalność** — uwzględnij tylko wtedy, gdy istnieje specyficzna metoda weryfikacji lub potrzeba instrumentacji wykraczająca poza standardowe logowanie.
...
**Umowa**: [Interfejs, sygnatura, pole schematu, trasa, delta struktury plików lub niezmiennik, którego dotyczy zmiana. W przypadku edycji czysto prozą, nazwij sekcję lub nagłówek, którego dotyczy.

Fragment kodu pojawia się tutaj TYLKO wtedy, gdy zmiana jest nieoczywista — trudne wyrażenie regularne, nietypowe wywołanie API, nieintuicyjna kolejność, obejście znanego błędu lub umowa sygnatury, od której zależą inne części planu. W przypadku rutynowych edycji (dodanie pola, podłączenie obsługi, zastosowanie istniejącego wzorca), opisz umowę i zakończ. Domyślnie: brak fragmentu.]
...
**Uwaga implementacyjna**: Po zakończeniu tej fazy i pomyślnym przejściu wszystkich automatycznych weryfikacji, zatrzymaj się tutaj, aby uzyskać ręczne potwierdzenie od człowieka, że testowanie ręczne zakończyło się sukcesem, zanim przejdziesz do następnej fazy. Bloki faz używają zwykłych wypunktowań — odpowiadające im pola wyboru `- [ ]` dla tych elementów znajdują się w sekcji `## Postęp` na dole planu.
...
Sekcja Postęp jest mechaniczna — emituj jedno `### Faza N: <nazwa>` na fazę, z podsekcjami `#### Automatyczne` / `#### Ręczne` wyliczającymi każdy punkt Kryteriów Sukcesu z tej fazy jako `- [ ] <faza>.<indeks> <tytuł>`. Pomiń puste podsekcje. Same bloki faz zawierają zwykłe wypunktowania `- ` (bez pól wyboru); sekcja `## Postęp` jest jedynym miejscem, gdzie pojawiają się `[ ]` / `[x]`.
...
[2-3 zdania: co budujemy/robimy i motywacja. Jeśli brief ramowy był danymi wejściowymi, umieść tutaj dosłownie przeformułowane (lub potwierdzone) oświadczenie problemowe — to jest "dlaczego" w jego najostrzejszej formie.]

## Punkt wyjścia

[1-2 zdania: co istnieje dzisiaj, na czym ten plan się opiera lub co zmienia. Ugruntuj czytelnika w obecnym stanie, aby zrozumiał różnicę. Jeśli ramka to zbadała, podsumuj z jej Badania Hipotez, zamiast powtarzać.]
...
8. **Opisuj zamiar, a nie implementację**:
   ...
   - W przypadku rutynowych edycji — dodawania pola, podłączania obsługi, stosowania istniejącego wzorca — opisz `**Cel**` w 1-2 zdaniach, nazwij `**Umowę**` w jednym i zakończ. Implementator (człowiek lub agent) ustala kod na podstawie ścieżki pliku, otaczającego wzorca i zamiaru
...
Kryteria sukcesu każdej fazy powinny używać pól wyboru `- [ ]` w nagłówkach `#### Weryfikacja automatyczna:` i `#### Weryfikacja ręczna:`.
...
Planowanie może być obciążone kontekstem ze względu na badania + iteracje. Utrzymuj kontekst efektywny:
...
- question: "Co użytkownik powinien widzieć podczas ładowania nowych elementów?"
  ...
  - label: "Spinner na całą stronę"
    ...
    multiSelect: false
- question: "Ile elementów powinno to obsługiwać bezproblemowo?"
  header: "Skala"
  options:
  ...
  - label: "Tysiące"
    description: "Paginacja oparta na kursorze + wirtualne przewijanie. · Mocna strona: Obsługuje wzrost bez spadku wydajności. · Kompromis: 2-3 razy więcej pracy implementacyjnej; zmienia umowę API."
...
- question: "Co uczeń powinien być w stanie ZROBIĆ po tym module — nie tylko wiedzieć?"
  ...
  - label: "Zalicz test wiedzy"
    ...
    multiSelect: false
- question: "Jak ten moduł powinien obsługiwać różne poziomy umiejętności w grupie odbiorców?"
  header: "Poziomy"
  options:
  - label: "Jedna ścieżka, zaawansowana"
    description: "Jedna ścieżka skierowana do doświadczonych programistów. · Mocna strona: Głęboka treść, brak prowadzenia za rękę, szanuje czas ekspertów. · Kompromis: Odstrasza początkujących — zrezygnują lub zaleją kanały wsparcia."
...
- question: "Jakie jest główne wąskie gardło w obecnym procesie biuletynu?"
  ...
  - label: "⭐ Recommended: Kuracja trwa zbyt długo"
    description: "Znajdowanie i ocenianie linków jest powolnym krokiem. · Mocna strona: Bezpośrednio celuje w czas do publikacji — automatyzacja kuracji daje największe oszczędności czasu na podstawie obecnych czasów procesu. · Kompromis: Automatyczna kuracja ryzykuje utratę osobistego głosu redakcyjnego, który cenią subskrybenci."
