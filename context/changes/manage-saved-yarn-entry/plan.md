...
- `deleteYarn(supabase, userId: string, id: string): Promise<void>` — najpierw odczytaj `photo_url` (jak wyżej); jeśli istnieje, best-effort `storage.remove([photo_url])` (warn, nie throw); następnie `.from("yarns").delete().eq("id", id).eq("user_id", userId).select("id")` i sprawdź, że dokładnie jeden wiersz został dopasowany — jeśli 0, rzuć błąd (nie znaleziono / nie należy do usera). Kaskada na `yarn_substitute_decisions` dzieje się automatycznie w DB.
